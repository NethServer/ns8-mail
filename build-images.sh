...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-mail; then
    ...
    buildah from --name nodebuilder-mail -v "${PWD}:/usr/src:Z" docker.io/library/node:24.19.0-slim
fi
...
buildah run "${container}" /bin/sh <<EOF
set -e
apk add --no-cache postfix gettext sqlite postfix-sqlite postfix-ldap openssl cyrus-sasl-login postsrsd
EOF
